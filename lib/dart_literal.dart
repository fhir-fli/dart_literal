...
class DartLiteralWriter {
  ...
  String _string(String s, int indent, {int firstLineUsed = 0}) {
    ...
    // makes it easier to search source files for a given path." The linter
    // exempts any slash, but the point is the one-token URI or path, so
    // only a string with no spaces stays whole; prose that happens to hold
    // a date or a unit ("mcg/0.3 mL") still splits at its spaces.
    if (!escaped.contains(' ') &&
